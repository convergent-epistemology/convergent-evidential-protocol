#!/usr/bin/env python3
"""
Shroud of Turin Convergence Map - v1.4 (Corrected)
Corrected implementation of v1.4:
- Gaussian copula now properly connected via beta.ppf()
- Bayes Factors use geometric mean (exp(mean log-odds))
- p_positive values are now dynamic
- Beta distributions for realistic uncertainty
- 3-layer domain weights
"""
...
def run_monte_carlo(likelihood_ranges, domain_weights, priors, beta_alpha, beta_beta, n_samples=8000, seed=42):
    ...
    for i in range(n_samples):
        u = sample_correlated_uniforms(1, corr_mat)[0]
        joint_L = {h: float(priors[h]) for h in hypotheses}
        ...
        for d_idx, dname in enumerate(domains):
            ...
            for h in hypotheses:
                ...
                # Corrected: Use copula draw with Beta PPF
                u_beta = beta.ppf(u_d, beta_alpha, beta_beta)
...
if __name__ == "__main__":
    print("=== SHROUD MC v1.4 (Corrected) ===\n")
    ...
    print("Step 3: Running Monte Carlo with Beta sampling + copula (8000 iterations)...")
    ...
    comparisons = []
    for pair in [("H_D vs H_A", "log_odds_HD_vs_HA"), ("H_C vs H_A", "log_odds_HC_vs_HA"), ("H_D vs H_C", "log_odds_HD_vs_HC")]:
        log_odds = np.array(results[pair[1]])
        mean_log_odds = float(np.mean(log_odds))
        mean_BF = float(np.exp(mean_log_odds))
        p_positive = round(float(np.mean(log_odds > 0)) * 100, 1)
        comparisons.append({
            "comparison": pair[0],
            "mean_log_odds": round(mean_log_odds, 3),
            "mean_BF": round(mean_BF, 2),
            "p_positive": p_positive
        })
    
    print("\n" + "="*70)
    print("POSTERIOR RESULTS (v1.4 Corrected)")
    print("="*70)
    ...
    for comp in comparisons:
        print(f"  {comp['comparison']}: {comp['mean_BF']:.2f}× (p_positive: {comp['p_positive']}%)")
    ...
    output = {
        "metadata": {
            ...
            "n_samples": 8000,
            "version": "v1.4 (Corrected)",
            "notes": "Corrected implementation: Gaussian copula active via beta.ppf(), geometric Bayes Factors, dynamic p_positive."
        },
    ...
    with open("/home/workdir/artifacts/shroud_mc_v1.4_results.json", "w") as f:
        json.dump(output, f, indent=2)
    
    print("\nResults saved to shroud_mc_v1.4_results.json")
    print("=== COMPLETE ===")
